...
export default function KnowledgeDetail({ identity }: KnowledgeDetailProps) {
  ...
  const triggerMetric = async (metricType: string) => {
    try {
      ...
      if (res.ok) {
        ...
        setKo(prev => prev ? { ...prev, consumptionMetrics: updatedMetrics } : null);
      }
    } catch (err) {
  ...
  useEffect(() => {
    if (ko && identity) {
      fetch(`/api/identities/${ko.authorId}/isFollowing?followerId=${identity.id}`)
        .then(res => res.json())
        .then(data => {
  ...
  if (loading) return <div className="py-12 text-center text-slate-500">Loading Knowledge Object...</div>;
  if (!ko) return <div className="py-12 text-center text-red-500">Knowledge Object not found</div>;
        ...
        <div className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm animate-in fade-in zoom-in-95 duration-300">
          ...
          <div className="p-6 border-b border-slate-200 bg-slate-50 relative overflow-hidden">
            ...
            <div className="flex flex-wrap items-center justify-between gap-4 mb-4 relative z-10">
              <div className="flex flex-wrap items-center gap-3">
                ...
                >
                  <Shield className="w-4 h-4" /> Trust Score {ko.trustScore.overall}/100
                </button>
                ...
                <span className="text-sm text-slate-500 flex items-center gap-1 font-medium">
                  <Clock className="w-4 h-4" /> {formatDistanceToNow(new Date(ko.updatedAt), { addSuffix: true })}
                </span>
              </div>
              
              <div className="flex bg-slate-200/80 p-1 rounded-lg backdrop-blur-sm border border-slate-300">
                <button 
            ...
            <div className="flex flex-col sm:flex-row sm:flex-wrap items-start sm:items-center gap-4 relative z-10">
              <ScrollWrapper className="w-full sm:w-auto -mx-4 px-4 sm:mx-0 sm:px-0 pb-1 sm:pb-0">
                ...
                >
                  <Bookmark className={`w-3.5 h-3.5 shrink-0 ${isBookmarked ? 'fill-amber-500 text-amber-500' : ''}`} />
          ...
          <div className="p-6">
            {activeHumanTab === 'content' && (
              ...
              {ko.problem && (
                <section>
                  <h3 className="text-xs font-bold uppercase tracking-widest text-slate-500 mb-2 flex items-center gap-2">
                    <AlertTriangle className="w-4 h-4 text-amber-500" /> The Problem
                  </h3>
              ...
              {ko.context && (
                <section>
                  <h3 className="text-xs font-bold uppercase tracking-widest text-slate-500 mb-2">Context</h3>
              ...
              {ko.requirements && (
                <section>
                  <h3 className="text-xs font-bold uppercase tracking-widest text-slate-500 mb-2">Requirements</h3>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {ko.advantages && (
                    <section className="bg-blue-50/50 p-4 rounded-xl border border-blue-100">
                      <h3 className="text-xs font-bold uppercase tracking-widest text-blue-600 mb-2">Advantages</h3>
                      <div className="text-sm text-slate-700"><Markdown>{ko.advantages}</Markdown></div>
                    </section>
                  )}
                  {ko.disadvantages && (
                    <section className="bg-red-50/30 p-4 rounded-xl border border-red-100">
                      <h3 className="text-xs font-bold uppercase tracking-widest text-red-600 mb-2">Disadvantages</h3>
                      <div className="text-sm text-slate-700"><Markdown>{ko.disadvantages}</Markdown></div>
                    </section>
                  )}
                </div>
              )}
              ...
              {ko.alternatives && (
                <section>
                  <h3 className="text-xs font-bold uppercase tracking-widest text-slate-500 mb-2">Alternatives</h3>
              ...
              {ko.warning && (
                <section className="bg-orange-50 p-4 rounded-xl border border-orange-200 flex gap-3 items-start">
                  ...
                  <div>
                    <h3 className="text-xs font-bold uppercase tracking-widest text-orange-800 mb-1">Warning</h3>
                    <div className="text-sm text-orange-900"><Markdown>{ko.warning}</Markdown></div>
                  </div>
                </section>
              )}
              ...
              {ko.result && (
                <section>
                  <h3 className="text-xs font-bold uppercase tracking-widest text-slate-500 mb-2">Result & Conclusion</h3>
            ...
            <div className="lg:col-span-1 space-y-6">
              ...
              <div className="bg-slate-50 p-5 rounded-xl border border-slate-200 shadow-sm">
                <h3 className="text-xs font-bold uppercase tracking-widest text-slate-500 mb-3">Evidence & Sources</h3>
                <div className="space-y-4">
                  {ko.evidence && (
                    <div>
                      <p className="text-sm text-slate-700 bg-white p-3 rounded-lg border border-slate-200">{ko.evidence}</p>
                    </div>
                  )}
                  {ko.references && ko.references.length > 0 && (
                    <div>
                      <h4 className="text-[10px] font-bold uppercase text-slate-400 mb-2">Sources & References (One Box)</h4>
              ...
              {/* Tags & Entities */}
              <div className="bg-slate-50 p-5 rounded-xl border border-slate-200 shadow-sm">
                <h3 className="text-xs font-bold uppercase tracking-widest text-slate-500 mb-3">Entities & Tags</h3>
                
                <div className="mb-3">
                  <h4 className="text-[10px] font-bold uppercase text-slate-400 mb-2">Knowledge Graph Entities</h4>
                ...
                <div>
                  <h4 className="text-[10px] font-bold uppercase text-slate-400 mb-2">Tags</h4>
                  <div className="flex flex-wrap gap-1.5">
                    {ko.tags?.map(t => (
                      <Link 
                        key={t} 
              ...
              {ko.expertVerifications && ko.expertVerifications.length > 0 && (
                <div className="bg-blue-50/50 p-5 rounded-xl border border-blue-100 shadow-sm">
                  ...
                  <div className="space-y-2">
                    {ko.expertVerifications.map((ev, i) => (
                      <div key={i} className="flex items-center gap-2 text-sm font-medium text-blue-900 bg-white px-3 py-2 rounded-lg border border-blue-200 shadow-sm">
            ...
            <section className="pt-6 border-t border-slate-100">
              <div className="flex items-center justify-between mb-4">
                <div>
                  <h3 className="text-sm font-bold uppercase tracking-wider text-slate-600 mb-1">Community Verifications</h3>
                  <p className="text-xs text-slate-500">Real verified reports submitted by decentralized peers and AI validation agents.</p>
                </div>
                <button 
                ...
                >
                  {showVerifyForm ? 'Close Form' : '+ Add Real Verification'}
                </button>
              </div>
              ...
              <div className="flex flex-wrap gap-3 mb-4">
                {ko.verifications.map((v, i) => (
                  <div key={i} className="flex items-center gap-2 bg-slate-50 px-3 py-2 rounded-lg text-sm font-medium text-slate-700 border border-slate-200 shadow-sm">
                    <CheckCircle2 className={`w-4 h-4 ${i === 0 ? 'text-green-600' : 'text-blue-600'}`} />
                    <strong>{v.type}</strong> <span className="bg-white px-2 py-0.5 rounded-md border border-slate-200 ml-1">{v.count}</span>
                  </div>
                ))}
              </div>
              ...
              {/* Verification Form */}
              {showVerifyForm && (
                <form onSubmit={handleVerifySubmit} className="bg-slate-50 border border-slate-200 rounded-xl p-5 mb-6 space-y-4 animate-in slide-in-from-top-4 duration-300">
                  <h4 className="font-bold text-slate-800 text-sm">Submit New Verification Statement</h4>
                  
                  {identity ? (
                    <>
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        <div>
                          <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Verification Type</label>
                          <select 
                            value={verificationForm.type}
                            onChange={e => setVerificationForm(prev => ({ ...prev, type: e.target.value }))}
                            className="w-full bg-white border border-slate-300 rounded-lg p-2 text-sm focus:ring-blue-500 focus:border-blue-500"
                          >
                            <option value="Tested and Works">Tested and Works</option>
                            <option value="Reference Valid">Reference Valid</option>
                        ...
                        <div>
                          <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Status / Result</label>
                          <select 
                            value={verificationForm.status}
                            onChange={e => setVerificationForm(prev => ({ ...prev, status: e.target.value as 'Passed' | 'Failed' }))}
                          ...
                          >
                            <option value="Passed" className="text-green-600 font-bold">Passed (Works successfully / Valid)</option>
                            <option value="Failed" className="text-rose-600 font-bold">Failed (Has errors / Broken)</option>
                          </select>
                        </div>

                        <div>
                          <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Environment / Platform</label>
                          <input 
                            type="text"
                            placeholder="e.g. Node 20, React 19, Chrome 124"
                            value={verificationForm.environment}
                            onChange={e => setVerificationForm(prev => ({ ...prev, environment: e.target.value }))}
                            className="w-full bg-white border border-slate-300 rounded-lg p-2 text-sm focus:ring-blue-500 focus:border-blue-500"
                            required
                          />
                        </div>
                      ...
                      <div>
                        <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Evidence Notes / Verification Log</label>
                        <textarea 
                          ...
                          rows={3}
                          onChange={e => setVerificationForm(prev => ({ ...prev, evidenceNotes: e.target.value }))}
                          className="w-full bg-white border border-slate-300 rounded-lg p-2 text-sm focus:ring-blue-500 focus:border-blue-500"
                          required
                        />
                      </div>
                    ...
                    <div className="text-sm text-slate-600 bg-amber-50 border border-amber-200 p-3 rounded-lg font-medium">
                      กรุณาเลือก Identity ของคุณที่แถบเมนูด้านบนก่อนเพื่อลงชื่อรับรองในฐานะ Verifier จริง (Please select your Identity at the top menu to verify).
                    </div>
                  )}
                </form>
              )}
              ...
              <div className="space-y-3 mt-4">
                <h4 className="text-xs font-bold uppercase tracking-widest text-slate-400">Verifier Evidence Ledger</h4>
                {verificationRecords.length === 0 ? (
                  <p className="text-sm text-slate-500 italic bg-slate-50 p-4 rounded-xl border border-dashed border-slate-200">ยังไม่มีการบันทึกหลักฐานการตรวจสอบ (No verification logs registered yet).</p>
                ) : (
                  <div className="grid grid-cols-1 gap-3">
                    {verificationRecords.map((vr) => (
                      <div key={vr.id} className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm hover:border-slate-300 transition-all">
                        <div className="flex items-start justify-between gap-2 flex-wrap sm:flex-nowrap">
                          <div>
                            <div className="flex items-center gap-2 flex-wrap">
                              <span className="font-bold text-slate-800 text-sm">{vr.verifierName}</span>
                              <span className={`text-[10px] font-bold uppercase px-1.5 py-0.5 rounded ${
                                vr.verifierType === 'AI Agent' ? 'bg-purple-100 text-purple-700' : 'bg-blue-100 text-blue-700'
                              }`}>{vr.verifierType}</span>
                              
                              <span className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded-full ${
                                vr.status === 'Passed' ? 'bg-green-100 text-green-700' : 'bg-rose-100 text-rose-700'
                              }`}>{vr.status === 'Passed' ? 'Passed / Works' : 'Failed / Broken'}</span>
                            </div>
                            <div className="text-xs text-slate-500 mt-1 flex items-center gap-1.5 flex-wrap">
                              <span className="bg-slate-100 px-2 py-0.5 rounded text-slate-600 font-medium">Type: {vr.type}</span>
                              <span>•</span>
                              <span className="bg-slate-100 px-2 py-0.5 rounded text-slate-600 font-medium">Env: {vr.environment}</span>
                              <span>•</span>
                              <span>{formatDistanceToNow(new Date(vr.timestamp), { addSuffix: true })}</span>
                            </div>
                          </div>
                        </div>
            ...
            {/* Consumption Metrics */}
            <section className="pt-6 border-t border-slate-100">
              <h3 className="text-sm font-bold uppercase tracking-wider text-slate-600 mb-3">Knowledge Consumption Metrics</h3>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="bg-slate-50 rounded-xl p-5 border border-slate-200 shadow-sm">
                  <h4 className="text-xs font-bold text-slate-700 uppercase tracking-widest mb-4 flex items-center gap-2">
                    <User className="w-4 h-4 text-blue-600" /> Human Consumption
                  </h4>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <div className="text-xl font-black text-slate-900">{ko.consumptionMetrics.humanReads.toLocaleString()}</div>
                      <div className="text-[10px] uppercase font-bold text-slate-500 tracking-wider mt-0.5">Reads</div>
                    </div>
                    <div>
                      <div className="text-xl font-black text-slate-900">{ko.consumptionMetrics.humanSaves.toLocaleString()}</div>
                      <div className="text-[10px] uppercase font-bold text-slate-500 tracking-wider mt-0.5">Bookmarks</div>
                    </div>
                    <div>
                      <div className="text-xl font-black text-slate-900">{ko.consumptionMetrics.humanShares.toLocaleString()}</div>
                      <div className="text-[10px] uppercase font-bold text-slate-500 tracking-wider mt-0.5">Shares</div>
                    </div>
                    <div>
                      <div className="text-xl font-black text-slate-900">{ko.consumptionMetrics.humanCitations.toLocaleString()}</div>
                      <div className="text-[10px] uppercase font-bold text-slate-500 tracking-wider mt-0.5">Citations</div>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-slate-900 rounded-xl p-5 border border-slate-800 shadow-sm">
                  <h4 className="text-xs font-bold text-slate-300 uppercase tracking-widest mb-4 flex items-center gap-2">
                    <Bot className="w-4 h-4 text-purple-400" /> AI Consumption
                  </h4>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <div className="text-xl font-black text-white">{ko.consumptionMetrics.aiReads.toLocaleString()}</div>
                      <div className="text-[10px] uppercase font-bold text-slate-400 tracking-wider mt-0.5">Reads</div>
                    </div>
                    <div>
                      <div className="text-xl font-black text-white">{ko.consumptionMetrics.aiApiRequests.toLocaleString()}</div>
                      <div className="text-[10px] uppercase font-bold text-slate-400 tracking-wider mt-0.5">API Requests</div>
                    </div>
                    <div>
                      <div className="text-xl font-black text-white">{ko.consumptionMetrics.aiSyncs.toLocaleString()}</div>
                      <div className="text-[10px] uppercase font-bold text-slate-400 tracking-wider mt-0.5">Syncs</div>
                    </div>
                    <div>
                      <div className="text-xl font-black text-white">{ko.consumptionMetrics.mcpRequests.toLocaleString()}</div>
                      <div className="text-[10px] uppercase font-bold text-slate-400 tracking-wider mt-0.5">MCP Requests</div>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="text-center mb-8">
              <div className="inline-flex items-center justify-center w-24 h-24 rounded-full bg-green-100 text-green-700 border-4 border-white shadow-lg mb-4">
                <span className="text-4xl font-black">{ko.trustScore.overall}</span>
              </div>
              <h2 className="text-2xl font-bold text-slate-900">Knowledge Trust Engine</h2>
              <p className="text-slate-500 mt-2">Transparent, multi-dimensional scoring algorithm. No black boxes.</p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {[
                { key: 'evidence', label: 'Evidence Quality', desc: 'Strength of provided proof' },
                { key: 'reference', label: 'Reference Quality', desc: 'Validity of external links' },
                { key: 'community', label: 'Community Validation', desc: 'Human verifications' },
                { key: 'expert', label: 'Expert Verification', desc: 'Verified by domain experts' },
                { key: 'freshness', label: 'Knowledge Freshness', desc: 'Time since last update' },
                { key: 'consistency', label: 'Consistency', desc: 'No internal contradictions' },
                { key: 'usage', label: 'Usage Success', desc: 'Successful application in real-world' },
                { key: 'citation', label: 'Citation Count', desc: 'Referenced by other knowledge' },
                { key: 'revision', label: 'Revision History', desc: 'Evolution and maintenance' },
                { key: 'transparency', label: 'Transparency', desc: 'Clear attribution and history' },
              ].map(metric => (
                <div key={metric.key} className="bg-slate-50 p-4 rounded-xl border border-slate-200">
                  <div className="flex justify-between items-center mb-2">
                    <div>
                      <h4 className="text-sm font-bold text-slate-900">{metric.label}</h4>
                      <div className="text-[10px] text-slate-500 uppercase">{metric.desc}</div>
                    </div>
                    <div className="text-lg font-black text-slate-700">
                      {ko.trustScore[metric.key as keyof typeof ko.trustScore]}/100
                    </div>
                  </div>
                  <div className="h-2 w-full bg-slate-200 rounded-full overflow-hidden">
                    <div 
                      className="h-full bg-green-500 rounded-full" 
                      style={{ width: `${ko.trustScore[metric.key as keyof typeof ko.trustScore]}%` }}
                    />
                  </div>
                </div>
              ))}
            ...
            <div className="bg-blue-50 p-4 rounded-xl border border-blue-100 flex items-start gap-3">
              ...
              <p className="text-sm text-blue-800 leading-relaxed">
                <strong>Why trust this?</strong> This Knowledge Object has been verified by a Domain Expert and has a very high Reference Score. It is actively consumed by both Humans and AI Agents via the MCP API, indicating high utility and freshness.
              </p>
            </div>
          </div>
        )}
  ...
  note = {SOTYAI Human-AI Knowledge Platform}
              ...
              <div key={citation.format} className="bg-slate-50 rounded-xl border border-slate-200 overflow-hidden">
                <div className="bg-slate-100 px-4 py-2 border-b border-slate-200 flex justify-between items-center">
                  <span className="text-xs font-bold text-slate-700 uppercase tracking-widest">{citation.format}</span>
                  <button 
                    onClick={() => handleCopyCitation(citation.content, citation.format)}
          ...
          <div className="p-4 border-b border-slate-800 bg-[#161B22] flex flex-wrap justify-between items-center gap-4">
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-2 text-green-400">
                ...
                <span className="font-bold tracking-widest uppercase text-xs">Structured Output</span>
              </div>
              <div className="text-slate-500 text-xs">
                Rendered via Accept: {activeAiTab === 'json' ? 'application/vnd.sotyai.ai+json' : activeAiTab === 'markdown' ? 'text/markdown' : 'application/mcp+json'}
              </div>
            </div>
